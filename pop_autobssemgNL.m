...
function [EEG, com] = pop_autobssemgNL(EEG, wl, ws, bss_alg, bss_opt, crit_alg, crit_opt)
...
if EEG.trials == 1
    datais = 'Continuous';    
    def_w2 = 1+(rem(EEG.xmax,1)/floor(EEG.xmax));
    userdata.Cur = 1;
else
    datais = 'Epoched';
    def_w2 = (abs(EEG.xmax)+abs(EEG.xmin))*2;
    userdata.Cur = 2;
end
userdata.T = (abs(EEG.xmax)+abs(EEG.xmin))*EEG.trials;
userdata.TT =  EEG.trials;
userdata.TO = def_w2;
...
if showpopup
    ...
    uilist = { { 'style' 'text'      'string'    'BSS algorithm:'} { 'style' 'popupmenu' 'string'    selectalg} ...
        ...
        {'style'  'text'      'string'    strcat('Number of Epoch:',{' '}, num2str(EEG.trials)) 'fontweight', 'bold'} ...
...
%     result = inputgui( uigeom, uilist, 'pophelp(''pop_autobssemg'')', guititle, [],'normal');
    result = inputgui( 'geometry', uigeom, 'uilist', uilist, 'helpcom', ...
        'pophelp(''pop_autobssemgNL'');', 'title', guititle,'mode', 'normal', 'geomvert',[], ...
        'userdata', userdata);
